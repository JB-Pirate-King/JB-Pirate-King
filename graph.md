...
피처 추천 + 노드별 판정 + FE 분해 + Google Sheets 포함 최종 설계.
...
2. **노드별 판정**(보라): 각 compute 노드 뒤 claude 판정 → 분석·판정(continue/retry/stop).
...
graph TD
  ...
  g1 -->|stop| stop
  build --> hbuild["판정: 빌드로그"]
  hbuild -->|ok| g2{"게이트: 커밋·릴리즈?"}
  hbuild -->|stop| stop
  g2 -->|approve| rel[release · 커밋+릴리즈]
  g2 -->|stop| stop
  rel --> hrel["판정: 릴리즈·회귀 점검"]
  hrel --> ldone["Sheets: run_done"]
  ldone --> chain[chain · fe_state]
  chain --> hchain["판정: 다음 브랜치 전략"]
  hchain --> nb

  gC -->|approve| conv[converge · 수렴] --> hconv["판정: 종합 회고"] --> END([종료])
  gC -->|stop| stop
  stop --> END

  classDef core fill:#dcfce7,stroke:#16a34a;
  classDef judge fill:#ede9fe,stroke:#7c3aed;
  ...
  class hnb,hbase,hreco,hscan,hretr,himp,hfeval,hfe,hbuild,hrel,hchain,hconv judge;
  class g1,g2,gC,adopt gate;
  class lr,lfe,ldone sheet;
  class reco reco;
```

- 분홍=claude 피처추천 / 보라=판정 / 초록=compute / 노랑=게이트·결정 / 파랑=Sheets
...
| **judge** | j_* | claude -p 분석·판정 (노드별 on/off) |
...
| 항목 | 내용 |
|---|---|
| 입력 | 약세 진단(j_baseline) + 시도/채택 피처(중복회피) + 현재 FP1/5/10 |
...
def n_recommend(state):
    weak  = state["judge"]["baseline"]["evidence"]
...
def claude_judge(stage, ctx_fn):
    def node(state):
        text, extra = ctx_fn(state)
        v = json.loads(subprocess.run(["claude","-p",build_judge_prompt(stage,text,extra),
        ...
        return {"judge": {**state.get("judge",{}), stage: v}, "decision": v["verdict"]}
    return node

# 비용 제어: 켤 노드만 (가벼운 노드는 OFF/룰베이스)
JUDGE_ON = {"baseline","reco","scan","fe","build"}
def maybe_judge(stage, ctx_fn):
    return claude_judge(stage, ctx_fn) if stage in JUDGE_ON else passthrough
```
...
| j_* | `claude_judge(stage, ctx_fn)` | JUDGE_ON 토글 |
