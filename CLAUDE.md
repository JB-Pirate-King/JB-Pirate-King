...
**승인** (`_wait`):
- `--auto_approve` **ON**: 요약에 `❌` 있으면 자동 `stop`, 아니면 자동 `approve`
  (Slack에 `🤖 [auto_approve] {단계} → approve` 로그만)
- **OFF**: `wait_approval` → Slack 버튼 대기 → **✅ 다음 단계 / 🔄 다시 실행 / ❌ 중단**
- 채택이 발생하면(FE) 승인 없이 자동으로 다음 브랜치 진행, 수렴(채택 없음) 시에만 종료 확인.
