...
import sys
if r"C:\pl" not in sys.path:
    sys.path.insert(0, r"C:\pl")   # onnx 짧은 경로 설치 위치
...
def get_best_device(requested: str = "auto") -> torch.device:
    ...
    if requested not in ("auto", "cuda", "dml"):
...
def load_normal_from_csv(path: str, max_seqs: int = 15000,
    ...
    """CSV(또는 디렉터리) 에서 raw(스케일링 전) 정상 시퀀스 로드 — 두 번 패스 스트리밍"""
    # 파일 목록 결정
    if os.path.isdir(path):
        files = sorted(glob.glob(os.path.join(path, "*_preprocessed.csv")))
    elif "*" in path:
    ...
    print(f"  [정상] {len(files)}개 CSV 로드 중...")
    ...
    # MMSI 샘플링 (기본 3000으로 제한: 186파일 OOM 방지)
    n_mmsi = max_mmsi if max_mmsi else (min(3000, len(all_mmsi)))
    if len(all_mmsi) > n_mmsi:
        sampled_mmsi = set(random.sample(sorted(all_mmsi), n_mmsi))
    else:
        sampled_mmsi = all_mmsi

    MAX_RECS_PER_MMSI = 800   # MMSI당 최대 레코드 (메모리 보호)
        ...
        with open(fpath, encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                if mmsi not in sampled_mmsi:
                    continue
                if len(mmsi_data[mmsi]) >= MAX_RECS_PER_MMSI:
                    continue
                try:
...
def build_dataset(args):
    ...
    # ── 정상 시퀀스: 현재 디렉터리의 *_preprocessed.csv 전부 사용 ─
    part_files = sorted(glob.glob("*_preprocessed.csv"))
    csv_candidates = part_files if part_files else [
...
def make_loaders(X_t, y_t, batch_size: int, val_ratio: float):
    ...
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, drop_last=True,  pin_memory=pin, num_workers=0)
...
def export_onnx(model, name: str, device):
    ...
    cpu_wrapped = SigmoidWrapper(model.cpu()).eval()
    dummy = torch.zeros(1, SEQ_LEN, N_FEAT)   # CPU
    model_out_dir = os.path.join(OUTPUT_DIR, f"sup_{name}")
    os.makedirs(model_out_dir, exist_ok=True)
    path = os.path.join(model_out_dir, f"model_sup_{name}.onnx")
    pt_path = os.path.join(model_out_dir, f"model_sup_{name}.pt")

    # ONNX export 시도 (실패 시 .pt 폴백)
    for opset in (13, 11):
        try:
            torch.onnx.export(
                cpu_wrapped, dummy, path,
                input_names=["x"],
                output_names=["output"],
                opset_version=opset,
                do_constant_folding=True,
                dynamic_axes={"x": {0: "batch", 1: "seq"}, "output": {0: "batch"}},
            )
            print(f"  ONNX export 완료 (opset={opset}): {path}")
            model.to(device)
            return path
        except Exception as e:
            print(f"  ONNX opset={opset} 실패: {e.__class__.__name__} -- 다음 시도...")

    # 모든 opset 실패 시 PyTorch 형식으로 저장
    print(f"  ONNX export 불가 -- PyTorch 모델로 저장: {pt_path}")
    torch.save(cpu_wrapped.state_dict(), pt_path)
    model.to(device)
    return pt_path
...
def main():
    ...
    results = []
    for name in targets:
        # 이미 완료된 모델 스킵 (ONNX 또는 PT 파일 존재 시)
        out_dir = os.path.join(OUTPUT_DIR, f"sup_{name}")
        onnx_done = os.path.join(out_dir, f"model_sup_{name}.onnx")
        pt_done   = os.path.join(out_dir, f"model_sup_{name}.pt")
        if os.path.exists(onnx_done) and os.path.getsize(onnx_done) > 0:
            print(f"\n[스킵] {name} -- 이미 ONNX 완료: {onnx_done}")
            continue
        if os.path.exists(pt_done) and os.path.getsize(pt_done) > 0:
            print(f"\n[스킵] {name} -- 이미 PT 완료: {pt_done}")
            continue
    ...
    if results:
        ...
        print("  (이번 실행에서 새로 학습된 모델 없음 -- 모두 스킵 또는 오류)")
    print(f"{'='*60}")
