...
def _write_run_report(deploy_dir: Path, branch: str, args, full_extra: list):
    """run 결과 리포트 README 생성 — ml/deploy/{branch}/README.md.

    FE 산출 JSON(WORK_DIR/feat_eng_iter01.json)에서 탐지율·시나리오·중요도를 읽어
    모델 파일 옆에 사람이 읽는 성능 문서로 남긴다. JSON 없으면 None (리포트 생략)."""
    fe_json = WORK_DIR / "feat_eng_iter01.json"
    if not fe_json.exists():
        return None
    try:
        d = json.loads(fe_json.read_text(encoding="utf-8"))
    except Exception as e:
        print(f"[run리포트] FE JSON 파싱 실패(생략): {e}")
        return None

    base = d.get("baseline_det")
    det = d.get("best_det")
    gain = (det - base) if (det is not None and base is not None) else None
    descs = d.get("feature_descriptions", {})
    adopted = d.get("best_extra", [])

    lines = [
        f"# {branch} — Run Report",
        "",
        f"- Generated: {time.strftime('%Y-%m-%d %H:%M:%S')}",
        f"- Model: `{args.model}` (epochs={args.epochs}, max_mmsi={args.max_mmsi})",
        f"- Data: `{args.data_file}`",
        f"- Release tag: `run/{branch}`",
        "",
        "## Detection",
        "",
        "| Baseline FP=1% | Final FP=1% | Δ | FP=5% | FP=10% | Deploy threshold |",
        "|---|---|---|---|---|---|",
        "| {} | {} | {} | {} | {} | {} |".format(
            f"{base:.1f}%" if base is not None else "-",
            f"{det:.1f}%" if det is not None else "-",
            f"{gain:+.1f}pp" if gain is not None else "-",
            f"{d.get('det_fp5', 0):.1f}%" if d.get("det_fp5") is not None else "-",
            f"{d.get('det_fp10', 0):.1f}%" if d.get("det_fp10") is not None else "-",
            f"{d.get('threshold'):.8f}" if d.get("threshold") is not None else "-",
        ),
        "",
        "## Features",
        "",
        f"- Total: **{len(BASE_FEATURES) + len(full_extra)}** (base {len(BASE_FEATURES)} + extra {len(full_extra)})",
        f"- Adopted this run: " + (", ".join(f"`{f}`" for f in adopted) if adopted else "(none)"),
    ]
    for f in adopted:
        if descs.get(f):
            lines.append(f"  - `{f}` — {descs[f]}")
    if full_extra:
        lines += ["- Cumulative extras: " + ", ".join(f"`{f}`" for f in full_extra)]

    sc = d.get("scenario_fp1") or {}
    if sc:
        lines += ["", "## Per-scenario detection (FP=1%)", "",
                  "| Scenario | Det % | |", "|---|---|---|"]
        for name, v in sorted(sc.items(), key=lambda kv: kv[1]):
            flag = "⚠️ weak" if v < 50 else ""
            lines.append(f"| {name} | {v:.1f}% | {flag} |")

    imp = d.get("permutation_importance") or []
    if imp:
        lines += ["", "## Permutation importance (top 10, more negative = more important)", "",
                  "| Feature | Δdet (pp) |", "|---|---|"]
        for item in sorted(imp, key=lambda x: x.get("importance", 0))[:10]:
            lines.append(f"| {item.get('feature','?')} | {item.get('importance', 0):+.2f} |")

    lines += [
        "", "## Deployment",
        "",
        "Rename to the plugin's fixed names and place under the runtime data dir:",
        "```bash",
        ...
        "```",
        "",
        "🤖 Generated by orchestrator.py (stage_release)",
    ]
    deploy_dir.mkdir(parents=True, exist_ok=True)   # 소급/단독 호출 대비
...
def stage_release(bot, args, branch: str, run_num: int,
    ...
    # run 결과 리포트 (탐지율·시나리오·중요도) — 모델 파일 옆 README 로 함께 커밋
    report = _write_run_report(deploy_dir, branch, args, full_extra)
    if report:
        deploy_files.append(report)
    if deploy_files:
