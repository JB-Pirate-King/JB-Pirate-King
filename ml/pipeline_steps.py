...
def _release_notes(args, branch: str, run_num: int, full_extra: list, det_rate) -> str:
    """릴리즈 노트 — FE JSON(`feat_eng_iter{NN}.json`)을 읽어 오탐율별(FP=1/5/10%) 탐지율,
    시나리오별 탐지율, 피처 중요도 표를 포함한 상세 노트를 만든다. JSON 없으면 기본 정보만."""
    n_feat    = len(BASE_FEATURES) + len(full_extra)
    feat_list = ", ".join(f"`{f}`" for f in full_extra) if full_extra else "-"
    det_str   = f"{det_rate:.1f}" if det_rate is not None else "?"
    fmt = lambda x: f"{x:.1f}%" if isinstance(x, (int, float)) else "?"

    d = {}
    fe_json = WORK_DIR / f"feat_eng_iter{run_num:02d}.json"
    if fe_json.exists():
        try:
            d = json.loads(fe_json.read_text(encoding="utf-8"))
        except Exception:
            d = {}
    base, fp5, fp10, thr = (d.get("baseline_det"), d.get("det_fp5"),
                            d.get("det_fp10"), d.get("threshold"))

    head = [
        f"## 모델", f"- {args.model}  (epochs={args.epochs})", "",
        f"## 학습 데이터", f"- {args.data_file}", "",
        f"## 피처 ({n_feat}개)", f"- 기본 {len(BASE_FEATURES)}개 + 추가: {feat_list}", "",
        f"## 브랜치", f"- {branch}", "",
    ]
    perf = ["## 성능 — 오탐율별 최종 탐지율", "",
            "| FP target | 탐지율 |", "|---|---|",
            f"| FP=1% | {det_str}% |", f"| FP=5% | {fmt(fp5)} |", f"| FP=10% | {fmt(fp10)} |", ""]
    if isinstance(base, (int, float)) and det_rate is not None:
        perf.append(f"- 베이스라인(FP=1%): {base:.1f}% → {det_rate:.1f}% ({det_rate - base:+.1f}pp)")
    if isinstance(thr, (int, float)):
        perf.append(f"- 배포 threshold (FP=1% 정상 99th pct): `{thr:.8f}`")
    perf.append("")

    scen = d.get("scenario_fp1", {})
    scen_tbl = []
    if scen:
        scen_tbl = ["## 시나리오별 탐지율 (FP=1%)  — ⚠️=약세(<50%)", "",
                    "| 시나리오 | 탐지율 |", "|---|---|"]
        for n, v in scen.items():
            warn = " ⚠️" if isinstance(v, (int, float)) and v < 50 else ""
            scen_tbl.append(f"| {n} | {fmt(v)}{warn} |")
        scen_tbl.append("")

    perm = d.get("permutation_importance", [])
    imp_tbl = []
    if perm:
        imp_tbl = ["## 피처 중요도 (permutation — 제거 시 탐지율 변화 pp, 더 음수일수록 중요)", "",
                   "| 피처 | importance (pp) |", "|---|---|"]
        for it in perm:
            f_, imp = it.get("feature"), it.get("importance")
            imp_tbl.append(f"| `{f_}` | {imp:+.1f} |" if isinstance(imp, (int, float))
                           else f"| `{f_}` | ? |")
        imp_tbl.append("")

    deploy = [
        "## 배포 (Deployment)",
        "이 릴리스의 모델 3파일은 학습 산출물 이름이라 그대로는 플러그인이 로드하지 않음.",
        "**런타임 로드 위치**(`g_pData`)로 옮기고 **고정 이름으로 리네임**해야 함:",
        "- 런타임 경로 (Linux): `~/.opencpn/plugins/ais_ids_pi/data/`",
        "- 로드 규칙 (`ais_ids.cpp`): `ensemble_config.json` 없으면 → `model.onnx`/`scaler.json`/`threshold.txt` 단일모델 폴백",
        "",
        "```bash",
        ...
        "",
        "## 첨부 (Assets)",
        "- 모델 3파일 (onnx/scaler/threshold)",
        "- 실행 로그 (`{branch}_*.log`, nodeio) · Google Sheets CSV (해당 브랜치 행)",
        "",
        "🤖 Generated by orchestrator.py",
    ]
    return "\n".join(head + perf + scen_tbl + imp_tbl + deploy)


def stage_release(bot, args, branch: str, run_num: int,
                  full_extra: list, det_rate, sheet=None) -> None:
    """FE 채택 커밋 후 GitHub 릴리스 자동 생성.

    태그: run/{model}_{NNN} (브랜치명 기반, prerelease).
    첨부: 모델 3파일 + 실행 로그 + 시트 CSV(해당 브랜치 행) + tar.gz(있으면).
    노트: FP=1/5/10% 탐지율·시나리오·피처중요도 표 (`_release_notes`).
    """
    plugin_dir = Path("ais_ids_pi")
    model_dir  = WORK_DIR / "model"
    ...
    notes = _release_notes(args, branch, run_num, full_extra, det_rate)

    # 에셋: 모델3 + tar.gz + 실행 로그(브랜치별 + 런 nodeio/run) + 시트 CSV(브랜치 행)
    log_assets = []
    logs_dir = Path("ml/logs")
    if logs_dir.exists():
        for pat in [f"{branch}_*.log", f"nodeio_{args.model}_*.log", f"run_{args.model}_*.log"]:
            hits = sorted(logs_dir.glob(pat))
            if hits:
                log_assets.append(str(hits[-1]))      # 최신 1개
    sheet_csvs = []
    if sheet is not None:
        try:
            sheet_csvs = sheet.export_branch_csv(args.model, branch, deploy_dir)
            if sheet_csvs:
                bot.log(f"📄 시트 CSV {len(sheet_csvs)}개 export (브랜치 행)", "릴리스")
        except Exception as e:
            bot.log(f"⚠ 시트 CSV export 실패(무시): {e}", "릴리스")

    # 코어(모델3+tar.gz)는 release create 에 인라인 첨부(안정), 로그·시트 CSV 는 별도 upload
    # (asset 하나라도 업로드 실패하면 create 전체가 실패하므로 부가 파일은 분리해 릴리즈를 지킨다).
    core_assets  = ([tarball] if tarball else []) + model_files
    extra_assets = log_assets + sheet_csvs
    ...
    cmd = ["gh", "release", "create", tag,
           ...
           "--notes", notes,
           "--target", commit_sha,
           "--prerelease"] + core_assets

    bot.log(f"📦 GitHub 릴리스 생성 중: {tag}  (코어 {len(core_assets)}개)", "릴리스")
    ret, out = run_cmd(cmd)
    if ret == 0:
        url_line = next((l for l in out.splitlines() if "github.com" in l), out.strip())
        if extra_assets:   # 로그·시트 CSV — best-effort, 실패해도 릴리즈 유지
            up_ret, up_out = run_cmd(["gh", "release", "upload", tag, "--clobber"] + extra_assets)
            if up_ret == 0:
                bot.log(f"📎 로그·시트 {len(extra_assets)}개 추가 첨부", "릴리스")
            else:
                bot.log(f"⚠ 로그·시트 첨부 실패(릴리즈는 유지)\n{up_out[-200:]}", "릴리스")
...
def _fe_commit_release(bot, sheet, branch, args, run_num, r: dict, commit_files: list):
    ...
    stage_release(bot, args, branch, run_num, r["full_extra"], r["det_rate"], sheet=sheet)
